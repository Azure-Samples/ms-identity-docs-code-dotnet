...
// Ideally, you'd first attempt to used a cached access token if one was available. This will renew 
// existing, but expired access tokens if possible. In this specific sample, this will always result
// in a cache miss, but this pattern would be what you'd use on subsequent calls that require the usage
// of the same same access token.
